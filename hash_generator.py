...
def compute_hash(data):
    """Вычисляет SHA-1 хеш для байтовых данных."""
...
def main():
    parser = argparse.ArgumentParser(description='Hash Generator (SHA-1)')
    ...
    print(f"{Fore.CYAN}🔐 Hash Generator (SHA-1) (Python)")
    print("Алгоритм: SHA-1")

    data = None
    if args.file:
        print(f"📂 Хеширование файла: {args.file}")
        try:
            hex_digest = hash_file(args.file)
        ...
        hex_digest = compute_hash(args.input.encode('utf-8'))
        data = hex_digest.encode()
    else:
        ...
        try:
            ...
            if not content:
                ...
                sys.exit(1)
            hex_digest = compute_hash(content)
            data = hex_digest.encode()
